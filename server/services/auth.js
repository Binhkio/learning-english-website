...
const login = async (email, password) => {
    ...
    const response = { user: user, jsonToken: jsonToken };
    return response;
};

const register = async (name, email, password, role) => {
    let user = await userDao.findUserByCondition({ email });
    if (!_.isNil(user)) throw new CustomApiMessage(httpCode.BAD_REQUEST, {}, 'User had been created');
    ...
    const jsonToken = await generateAccessToken(email, userId, role);
    const response = { user: newUser, jsonToken: jsonToken };
    return response;
};

const verifyCurrentUser = async (param, isAdmin = false) => {
    const currentUser = await userDao.findUserByCondition(param);
    if (_.isNil(currentUser)) throw new CustomApiMessage(httpCode.BAD_REQUEST, currentUser, '');
    if (isAdmin && currentUser.role !== constants.ADMIN) throw new CustomApiMessage(httpCode.BAD_REQUEST, {}, 'Not a admin');
};
